...
class TestRegister():
    ...
    def test_register(self, tclient, fuser):
        resp = tclient.post('/users/', data=user_schema.dumps(fuser).data, content_type='application/json')
        result = json.loads(resp.data)
...
class TestLogin():
    def test_no_email(self, tclient, no_email_user):
        with pytest.raises(ValidationError):
            resp = tclient.post('/users/login', data=user_schema.dumps(no_email_user).data, content_type='application/json')
            assert resp.status_code == 422

    def test_no_password(self, tclient, no_password_user):
        resp = tclient.post('/users/login', data=user_schema.dumps(no_password_user).data, content_type='application/json')
        assert resp.status_code == 422

    def test_login(self, tclient, created_user, tsession):
        assert tsession.query(User).one()

        resp = tclient.post('/users/login', data=user_schema.dumps(created_user).data, content_type='application/json')
        result = json.loads(resp.data)
    ...
    def test_leaved_user_login(self, tclient, leaved_user, tsession):
        ...
        resp = tclient.post('/users/login', data=user_schema.dumps(leaved_user).data, content_type='application/json')

        assert resp.status_code == 400
        assert resp.data == b'Leaved User.'

    def test_no_user_login(self, tclient, fuser):
        resp = tclient.post('/users/login', data=user_schema.dumps(fuser).data, content_type='application/json')
